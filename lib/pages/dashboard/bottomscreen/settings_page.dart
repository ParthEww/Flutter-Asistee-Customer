...
class SettingsPage extends GetView<DashboardController> {
      ...
      body: SafeArea(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              buildHeaderSection(onEditProfileCallBack: () {
            ...
            child:
              ...
              Text(
                ...
                style: TextStyles.text12Medium
                    .copyWith(color: AppColors.primary.withOpacityPrecise(0.6)),
              )
            ]),
          ),
        ),
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                      ...
                      padding: const EdgeInsets.only(
                          left: 4, top: 4, right: 12, bottom: 4),
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.circular(32),
                      ),
                      child: Row(
  ...
  Widget buildFirstSettingsSection() {
    return Container(
      ...
      child: ListView.separated(
        ...
        shrinkWrap: true,
        itemCount: controller.firstSettingsList.length,
        itemBuilder: (BuildContext context, int index) {
          return buildSettingsView(
              bean: controller.firstSettingsList[index],
              onTap: (settingFieldType) {
                if (settingFieldType == SettingFieldType.WALLET) {
                  controller.onGoToWallet();
                } else if (settingFieldType == SettingFieldType.ADMIN_CHAT) {
                } else if (settingFieldType == SettingFieldType.MY_ADDRESS) {
                  controller.onGoToMyAddress();
                } else {
                  controller.onGoToNotifications();
                }
              });
        },
            ...
            children: [
              Divider(
                ...
                color: AppColors.secondary
                    .withOpacityPrecise(0.4), // Customize color
              )
            ],
          );
        },
  ...
  Widget buildSecondSettingsSection() {
    return Container(
      ...
      child: ListView.separated(
        physics: const NeverScrollableScrollPhysics(),
        shrinkWrap: true,
        itemCount: controller.secondSettingsList.length,
        itemBuilder: (BuildContext context, int index) {
          return buildSettingsView(
              bean: controller.secondSettingsList[index],
              onTap: (settingFieldType) {
                if (settingFieldType == SettingFieldType.ABOUT_US ||
                    settingFieldType == SettingFieldType.TERMS_AND_CONDITIONS ||
                    settingFieldType == SettingFieldType.PRIVACY_POLICY) {
                  controller.onGoToCms(settingFieldType);
                } else if (settingFieldType == SettingFieldType.CONTACT_US) {
                  controller.onGoToContactUs();
                } else {
                  controller.onGoToFaqs();
                }
              });
        },
        separatorBuilder: (BuildContext context, int index) {
  ...
  Widget buildThirdSettingsSection() {
    return Container(
      ...
      child: ListView.separated(
        physics: const NeverScrollableScrollPhysics(),
        shrinkWrap: true,
        itemCount: controller.thirdSettingsList.length,
        itemBuilder: (BuildContext context, int index) {
          return buildSettingsView(
              bean: controller.thirdSettingsList[index],
              onTap: (settingFieldType) {});
        },
        separatorBuilder: (BuildContext context, int index) {
          return Column(
  ...
  Widget buildSettingsView(
      {required SettingFieldType bean,
      required Function(SettingFieldType) onTap}) {
    return GestureDetector(
      ...
      child: Padding(
        padding:
            const EdgeInsets.only(left: 18, top: 18, right: 24, bottom: 18),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            CustomCircleIcon(
              ...
              backgroundColor: bean == SettingFieldType.DELETE_ACCOUNT
                  ? AppColors.green
                  : AppColors.primary,
            ),
